...
class MarkUrlDownloadGui(object):
    def __init__(self, tkFrame):
        ...
        #subFm00
        tkinter.Label(subFm00, text='请输入您想要下载的网址，每个网址换行输入：', font=("楷体", 13, "bold")).grid()

        #subFm01
        self.tWebSites = ScrolledText.ScrolledText(subFm01, height=42, width=125,wrap="none")
        ...
        # tt.create_ToolTip(self.pdfCheckButton, "保存为PDF不会耗费太多时间。")

        self.lDldProcess = ttk.Progressbar(subFm02, orient='horizontal', length=500, mode='determinate')
        ...
        #subFm03
        tkinter.Button(subFm03, text='修改', font=('宋体', 10), command=self.setSaveDir).grid(row=3, column=0, padx=8, sticky=W)
        self.lSaveDir = tkinter.Label(subFm03, text="保存路径："+self.saveDir, font=("宋体", 10, "bold"),cursor="hand2", anchor = NW, width = 100)
        self.lSaveDir.grid(row=3, column=1, sticky=W)
    ...
    def startDownload(self):
        ...
        urls = self.tWebSites.get("1.0", "10000.end").split('\n')
    ...
    def webDwnldFunc(self, urls, savePdfEn):
        ...
        for idx,url in enumerate(urls):
            ...
            dld.url2pdf(url, self.wkhtmltopdfExe, webSaveDir, savePdfEn)
